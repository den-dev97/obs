...
1. **Создание свойств**: Вы можете добавить новое свойство к объекту.
    `let obj = {}; obj.newProperty = "value";`
    
2. **Чтение свойств**: Вы можете получить значение свойства объекта.
    `let value = obj.newProperty;`
    
3. **Изменение свойств**: Вы можете изменить значение существующего свойства.
    `obj.newProperty = "newValue";`
    
4. **Удаление свойств**: Вы можете удалить свойство из объекта.
    `delete obj.newProperty;`
    
5. **Проверка наличия свойства**: Вы можете проверить, существует ли свойство в объекте.
    `if ("newProperty" in obj) {     // свойство существует }`
    
6. **Перебор свойств**: Вы можете перебрать все свойства объекта.
    `for (let key in obj) {     console.log(key, obj[key]); }`
    
7. **Получение списка ключей**: Вы можете получить массив всех ключей объекта.
    `let keys = Object.keys(obj);`
    
8. **Получение списка значений**: Вы можете получить массив всех значений объекта.
    `let values = Object.values(obj);`
    
9. **Получение списка пар ключ-значение**: Вы можете получить массив пар ключ-значение.
    `let entries = Object.entries(obj);`
    
10. **Копирование объекта**: Вы можете создать поверхностную копию объекта.
...
***
## Всплытие (bubbling)
"Всплытие" (bubbling) в JavaScript - это механизм, по которому события распространяются от внутренних элементов страницы к внешним. Всплытие обеспечивает порядок, в котором события обрабатываются в DOM (Document Object Model).

При возникновении события (например, клика мышью или нажатия клавиши на клавиатуре), браузер сначала ищет элемент, на котором произошло событие, и затем передает это событие вверх по дереву DOM от этого элемента к корню документа. На каждом уровне дерева событие будет обрабатываться любыми обработчиками событий, которые привязаны к текущему элементу.
...
```
В этом примере, если вы кликните по кнопке, которая является "ребенком" (`child`), вы увидите в консоли два сообщения: "Child clicked!" и "Parent clicked!". Сначала событие click обрабатывается на кнопке, а затем оно "всплывает" до родительского элемента `div`.

Однако стоит отметить, что всплытие событий можно отменить, используя метод `event.stopPropagation()`, который предотвращает дальнейшее всплытие события.
***
...
myObject.myMethod(); // Выведет `myObject`
```
Значение `this` также можно явно задать с помощью методов `call`, `apply` или `bind`.
***
## `call`, `apply` и `bind`
...
CORS использует дополнительные HTTP-заголовки для информирования браузера и сервера о разрешенных источниках и типах запросов. В случае, если сервер допускает запросы с определенного источника, он добавит соответствующий заголовок (Access-Control-Allow-Origin) в свой ответ.

Тем не менее, CORS не безопасен в отсутствие строгого контроля и может вести к уязвимостям, таким как атаки CSRF (Cross-Site Request Forgery) или XSS (Cross-Site Scripting), если его неправильно настроить.
***
Веб-оптимизация - это широкий набор практик и методов, направленных на улучшение производительности сайтов и веб-приложений. Здесь рассмотрим некоторые из наиболее общих практик:

**1. Улучшение производительности сети:**

- **Минимизация HTTP-запросов:** Больше запросов = больше времени загрузки. Минимизация количества запросов может ускорить загрузку. Это можно сделать, например, объединяя CSS и JavaScript файлы.
- **Использование CDN:** Content Delivery Networks (CDN) хранят копии ваших ресурсов на серверах по всему миру, что обеспечивает более быструю загрузку для пользователей, которые находятся далеко от вашего основного сервера.
- **HTTP/2:** Эта версия протокола HTTP позволяет многократные запросы и ответы между клиентом и сервером в рамках одного TCP-соединения, улучшая производительность.

**2. Оптимизация ресурсов:**

- **Минификация и сжатие:** Минификация удаляет ненужные символы (например, пробелы и комментарии) из кода, сжатие уменьшает размер файла. Используйте инструменты, такие как UglifyJS для JavaScript и CSSNano для CSS.
- **Оптимизация изображений:** Используйте подходящие форматы изображений (например, WebP может быть более эффективным), сжимайте изображения и используйте техники "ленивой загрузки" для изображений, которые не нужны сразу при загрузке страницы.
- **Использование кэширования:** Браузеры могут кэшировать ресурсы, что позволяет быстро загружать повторно посещаемые страницы. Управляйте этим процессом с помощью HTTP-заголовков кэширования.

**3. Код и архитектура:**

- **Улучшение производительности JavaScript:** Обратите внимание на дорогостоящие операции, такие как манипуляции с DOM и большие вычислительные задачи. Используйте Web Workers для неблокирующего выполнения кода.
- **Критический CSS:** Это минимальное количество CSS, необходимое для отображения верхней части страницы. Он должен быть встроен непосредственно в HTML, чтобы пользователь мог быстро увидеть страницу, пока остальные стили загружаются.
- **Асинхронная загрузка:** Скрипты и стили, которые не являются критическими, можно загружать асинхронно, чтобы не блокировать рендеринг страницы.

**4. Профилирование и мониторинг:**

- **Использование инструментов профилирования:** Используйте инструменты, такие как Lighthouse и DevTools в Chrome, чтобы оценить производительность вашего сайта и выявить области для улучшений.
- **Мониторинг в реальном времени:** Используйте решения для мониторинга реального времени пользовательского опыта (Real User Monitoring, RUM), чтобы понять, как ваш сайт работает для конечных пользователей.

Помните, что оптимизация - это процесс. Он требует постоянного измерения, экспериментирования и улучшений.
***
## RegExp
