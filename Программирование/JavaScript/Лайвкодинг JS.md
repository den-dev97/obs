...
4. **Сумма массива**: Написать функцию, которая принимает массив чисел и возвращает их сумму.
```
...
5. **Анаграмма**: Написать функцию, которая принимает две строки и проверяет, являются ли они анаграммами друг для друга.

jsCopy code

`function isAnagram(str1, str2) {   let sortedStr1 = str1.toLowerCase().split('').sort().join('');   let sortedStr2 = str2.toLowerCase().split('').sort().join('');   return sortedStr1 === sortedStr2; }`
...
6. **Максимальное число в массиве**: Написать функцию, которая возвращает максимальное число в массиве.

jsCopy code

`function maxNumber(arr) {   return Math.max(...arr); }`
...
7. **Удаление дубликатов из массива**: Написать функцию, которая удаляет дубликаты из массива.
...
8. **Обращение строки**: Написать функцию, которая переворачивает строку.
...
9. **Поиск наибольшего общего делителя (НОД)**: Написать функцию, которая находит НОД двух чисел.
...
10. **Факториал**: Написать функцию, которая вычисляет факториал числа.
...
11. **Проверка простого числа**: Написать функцию, которая проверяет, является ли число простым.

jsCopy code

`function isPrime(num) {   for(let i = 2, sqrt = Math.sqrt(num); i <= sqrt; i++)     if(num % i === 0) return false;    return num > 1; }`
...
12. **Найти сумму чисел в строке**: Написать функцию, которая находит сумму всех чисел в строке.

jsCopy code

`function sumNumbersInString(str) {   let sum = 0;   let numbers = str.match(/\d+/g);   if (numbers) {     sum = numbers.reduce((acc, curr) => acc + Number(curr), 0);   }   return sum; }`

Функция использует регулярное выражение для поиска всех чисел в строке, а затем суммирует их с помощью метода `reduce`.

13. **Преобразование строки в верблюжий регистр (camelCase)**: Написать функцию, которая преобразует строку в верблюжий регистр.

jsCopy code

`function toCamelCase(str) {   return str.replace(/([-_]\w)/g, g => g[1].toUpperCase()); }`
...
14. **Сортировка пузырьком**: Написать функцию, которая сортирует массив с помощью алгоритма сортировки пузырьком.

jsCopy code

`function bubbleSort(arr) {   let len = arr.length;   for (let i = len-1; i >= 0; i--){     for(let j = 1; j <= i; j++){       if(arr[j-1] > arr[j]){           let temp = arr[j-1];           arr[j-1] = arr[j];           arr[j] = temp;        }     }   }   return arr; }`

Эта функция использует алгоритм сортировки пузырьком, который многократно проходит по массиву, меняет местами соседние элементы, если они в неправильном порядке.

15. **Найти все вхождения подстроки в строку**: Написать функцию, которая возвращает все вхождения подстроки в строку.

jsCopy code

`function findOccurrences(str, subStr) {   let occurrences = [];   let i = -1;   while ((i = str.indexOf(subStr, i+1)) != -1){     occurrences.push(i);   }   return occurrences; }`

Эта функция использует метод `indexOf` для нахождения вхождений подстроки в строку и возвращает их индексы.

16. **Создание массива уникальных значений из двух массивов**: Написать функцию, которая объединяет два массива в один массив уникальных значений.

jsCopy code

`function uniteUnique(arr1, arr2) {   return [...new Set([...arr1, ...arr2])]; }`
...
17. **Подсчет количества вхождений каждого слова в строке**: Написать функцию, которая подсчитывает количество вхождений каждого слова в строке.

jsCopy code

`function wordCount(str) {   let counts = {};   let words = str.split(' ');   words.forEach(function(word) {     if (counts[word]) {       counts[word]++;     } else {       counts[word] = 1;     }   });   return counts; }`

Функция разбивает строку на слова и для каждого слова увеличивает его счетчик в объекте `counts`.

18. **Удаление определенного элемента из массива**: Написать функцию, которая удаляет все вхождения определенного значения из массива.
...
19. **Рекурсивная функция для вычисления степени числа**: Написать функцию, которая вычисляет степень числа.
