...
Миксины в Vue.js представляют собой способ повторного использования логики в различных компонентах. Миксин — это объект, который может содержать любые опции компонента. Когда компонент использует миксин, все опции миксина "смешиваются" с собственными опциями компонента.
...
new Vue({
  mixins: [myMixin],
  created() {
    console.log('Привет от компонента!');
  }
});
```

В этом примере, при создании экземпляра Vue, в консоли будет выведено:

```
Привет от миксина!
Привет от компонента!
```
### Особенности:

1. **Слияние опций**: Если миксин и компонент содержат перекрывающиеся опции (например, методы с одинаковыми именами), то используется стратегия слияния. Для хуков жизненного цикла (например, `created`) оба метода будут вызваны.
2. **Локальные и глобальные миксины**: Миксины могут быть глобальными (регистрируются через `Vue.mixin()`) или локальными (передаются через опцию `mixins` в компоненте).
3. **Порядок вызова**: Хуки жизненного цикла миксина вызываются перед хуками самого компонента.
4. **Реактивность**: Миксины могут содержать реактивные свойства и будут корректно интегрироваться в систему реактивности Vue.

### Совместимость с Composition API
Если вы используете Composition API, миксины могут не быть наилучшим выбором для повторного использования логики, поскольку Composition API предоставляет более гибкие и мощные средства для этого. Однако, если у вас есть существующий код на миксинах или вы работаете с библиотеками, которые их используют, миксины по-прежнему будут работать.
```
import { ref, defineComponent } from 'vue';

const useHello = () => {
  const hello = () => {
    console.log('Привет от Composition API!');
  };
  return { hello };
};
...
В Vue 3, Composition API предоставляет несколько мощных средств для повторного использования и организации кода, которые являются альтернативой миксинам:

### 1. Composable Functions

Это функции, которые используют хуки Composition API (`ref`, `reactive`, `computed`, `watch`, `watchEffect` и т.д.) для инкапсуляции повторно используемой логики.
...
Этот механизм позволяет передавать данные или функции от родительского компонента к дочерним, минуя промежуточные уровни.
...
export default {
  setup() {
    provide('someFunction', () => console.log('Hello from parent'));
  }
}
...
Это просто функции, которые инкапсулируют какую-либо логику и могут использовать любые из вышеупомянутых средств.
...
export function useCustomHook(initialValue) {
  ...
  const doubledData = computed(() => data.value * 2);
  
  return { data, doubledData };
}
```

### 4. Teleport и Suspense

### Teleport

В Vue 3, Teleport предоставляет возможность перемещать элементы DOM из одного компонента в другую часть DOM-дерева, не нарушая при этом реактивные связи и состояние компонента. Это особенно полезно для модальных окон, всплывающих подсказок и других UI-элементов, которые должны "выходить" за рамки своего родительского компонента.
...
Здесь содержимое модального окна будет физически перемещено в элемент с `id="end-of-body"`, но реактивное состояние и события будут работать, как если бы этот контент оставался внутри компонента.

### Suspense

Suspense — это специальный компонент в Vue 3, который позволяет обрабатывать асинхронные зависимости в вашем компонентном дереве. Он может "ожидать" загрузки асинхронного компонента и показывать запасной контент во время ожидания.
...
Здесь `AsyncComponent` — это асинхронный компонент, который загружается динамически. Пока он загружается, будет отображаться контент из слота `fallback`.
...
***
# Какие хуки есть во vue 2 и vue 3
Во Vue 2 и Vue 3 существуют различные хуки жизненного цикла (lifecycle hooks), которые позволяют выполнить какой-либо код на различных этапах работы компонента.

### В Vue 2:

1. **`beforeCreate`**: вызывается до инициализации реактивности и событий.
2. **`created`**: вызывается после инициализации реактивности и событий.
3. **`beforeMount`**: вызывается перед первым монтированием шаблона в DOM.
4. **`mounted`**: вызывается после первого монтирования шаблона в DOM.
5. **`beforeUpdate`**: вызывается перед обновлением DOM.
6. **`updated`**: вызывается после обновления DOM.
7. **`beforeDestroy`**: вызывается перед уничтожением компонента.
8. **`destroyed`**: вызывается после уничтожения компонента.

### В Vue 3:

В Vue 3 добавлены новые хуки и изменена их номенклатура:

1. **`beforeCreate`** и **`created`**: остались без изменений.
2. **`beforeMount`** и **`mounted`**: остались без изменений.
3. **`beforeUpdate`** и **`updated`**: остались без изменений.
4. **`beforeUnmount`**: замена `beforeDestroy`.
5. **`unmounted`**: замена `destroyed`.

#### Новые хуки в Vue 3:
1. **`onRenderTracked`**: вызывается при отслеживании зависимости.
2. **`onRenderTriggered`**: вызывается при срабатывании реактивной зависимости, которая приводит к повторному рендеру.

В Vue 3 с Composition API добавились также функциональные возможности для реактивности и разделения логики, но это уже отдельная тема.
***
